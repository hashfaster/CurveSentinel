...
def is_valid_curve_address(address, network='mainnet'):
    ...
    curve_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Curve address
        return False

    if (address_version != curve_version):
        return False
...
# shims 'til we can fix the curved side
def SHIM_serialise_for_curved(sentinel_hex):
    from models import CURVED_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = CURVED_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in curved
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # curved expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    curved_hex = serialise(obj)
    return curved_hex


# shims 'til we can fix the curved side
def SHIM_deserialise_from_curved(curved_hex):
    from models import CURVED_GOVOBJ_TYPES

    # unpack
    obj = deserialise(curved_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('curve.conf')
