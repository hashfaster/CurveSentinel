...
def has_curve_conf():
    ...
    valid_curve_conf = False

    # ensure curve_conf exists & readable
    #
    # if not, print a message stating that Curve Core must be installed and
    # configured, including JSONRPC access in curve.conf
    try:
        f = io.open(config.curve_conf)
        valid_curve_conf = True
    except IOError as e:
        print(e)

    return valid_curve_conf
...
def main():
    ...
    if not has_curve_conf():
        print("CurveCore must be installed and configured, including JSONRPC access in curve.conf")
        sys.exit(1)
