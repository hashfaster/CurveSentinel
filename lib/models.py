...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, curved):
        golist = curved.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_curved(curved, item)
    ...
    def vote_delete(self, curved):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(curved, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, curved, signal, outcome):
        import curvelib
        ...
        output = curved.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = curvelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(curved, signal)

    def sync_network_vote(self, curved, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = curved.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = CURVED_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 curve addr, non-multisig
            if not curvelib.is_valid_curve_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Curve address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (curvelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/CurveDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import curvelib
        obj_data = curvelib.SHIM_serialise_for_curved(self.serialise())
    ...
    def prepare(self, curved):
        try:
            object_hash = curved.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = CURVED_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not curvelib.is_valid_curve_address(addr, config.network):
    ...
    def hash(self):
        import curvelib
        return curvelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = CURVED_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, curved):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - curved.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, curved):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - curved.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, curved):
        now = int(time.time())
        return (self.created_at < (now - curved.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, curved):
        if self.is_expired(curved):
            return False

        return True

    def is_deletable(self, curved):
        if self.is_expired(curved):
            return True
