...
class CurveDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some curved methods
    ...
    @classmethod
    def from_curve_conf(self, curve_dot_conf):
        from curve_config import CurveConfig
        config_text = CurveConfig.slurp_config_file(curve_dot_conf)
        creds = CurveConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = curvelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = curvelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = curvelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_curved_proto_version_with_sentinel_ping)
