...
def test_valid_curve_address():
    from curvelib import is_valid_curve_address

    main = valid_curve_address()
    test = valid_curve_address('testnet')

    assert is_valid_curve_address(main) is True
    assert is_valid_curve_address(main, 'mainnet') is True
    assert is_valid_curve_address(main, 'testnet') is False

    assert is_valid_curve_address(test) is False
    assert is_valid_curve_address(test, 'mainnet') is False
    assert is_valid_curve_address(test, 'testnet') is True


def test_invalid_curve_address():
    from curvelib import is_valid_curve_address

    main = invalid_curve_address()
    test = invalid_curve_address('testnet')

    assert is_valid_curve_address(main) is False
    assert is_valid_curve_address(main, 'mainnet') is False
    assert is_valid_curve_address(main, 'testnet') is False

    assert is_valid_curve_address(test) is False
    assert is_valid_curve_address(test, 'mainnet') is False
    assert is_valid_curve_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from curvelib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % curvelib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(curvelib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(curvelib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(curvelib.blocks_to_seconds(16616)) == 2612035
